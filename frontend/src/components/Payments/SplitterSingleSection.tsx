import avatar1 from "../../assets/thumb1.png";
import { getToken } from "../../utils/saveAuth";
import { SplitterSingleSectionProps } from "../../utils/types/interface";

const SplitterSingleSection: React.FC<SplitterSingleSectionProps> = ({
  splitDetails,
  removeSplitItem,
}) => {
  const currentUser = getToken();

  const handleRemoveItem = () => {
    removeSplitItem(splitDetails.email);
  };

  return (
    <>
      <div className="flex flex-wrap items-center cursor-pointer  rounded-lg w-full p-4">
        <img src={avatar1} className="w-12 h-12 rounded-full" />
        <div className="ml-4 flex-1">
          <p className="text-md text-gray-800 font-semibold">
            {splitDetails.name}
          </p>
          {currentUser.name !== splitDetails.name && (
            <p className="text-md text-gray-800">{splitDetails.email}</p>
          )}
          <p className="text-l text-gray-500 mt-0.5">
            {currentUser.name === splitDetails.name ? "You pay " : "Pays "} $
            {parseFloat(splitDetails?.share_amount).toFixed(2).toString()}
          </p>
        </div>
        {currentUser.name !== splitDetails.name && (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-3 ml-2 cursor-pointer shrink-0 fill-gray-400 hover:fill-red-500"
            viewBox="0 0 320.591 320.591"
            onClick={handleRemoveItem}
          >
            <path
              d="M30.391 318.583a30.37 30.37 0 0 1-21.56-7.288c-11.774-11.844-11.774-30.973 0-42.817L266.643 10.665c12.246-11.459 31.462-10.822 42.921 1.424 10.362 11.074 10.966 28.095 1.414 39.875L51.647 311.295a30.366 30.366 0 0 1-21.256 7.288z"
              data-original="#000000"
            ></path>
            <path
              d="M287.9 318.583a30.37 30.37 0 0 1-21.257-8.806L8.83 51.963C-2.078 39.225-.595 20.055 12.143 9.146c11.369-9.736 28.136-9.736 39.504 0l259.331 257.813c12.243 11.462 12.876 30.679 1.414 42.922-.456.487-.927.958-1.414 1.414a30.368 30.368 0 0 1-23.078 7.288z"
              data-original="#000000"
            ></path>
          </svg>
        )}
      </div>
    </>
  );
};

export default SplitterSingleSection;
